...
像 [Flowplay](https://flowplay.ai/)、[Wordware](https://www.wordware.ai/) 和 Rift 这样的工具原生支持常见设计模式（投票、集成、验证、"团队"），这将帮助更多开发者理解这些模式并将其用于构建 Agent。一个有用且有主见的开发工具可能是解锁基于这种强大 Agent 技术的下一波应用的最重要基础设施之一。
